...
const NavCounter = (props) => (
    <Nav className="settingInput" func={navHorizontal}>
        <Nav
            className={`arrowL ${!props.withinL ? 'greyed':''}`}
            style={{backgroundImage:`url(${sprites})`}}
            onClick={()=>{ props.withinL && props.clickL() }}
            func={ key => { key === 'enter' && props.withinL && props.clickL() }}
        />
        {props.value}
        <Nav
            className={`arrowR ${!props.withinR ? 'greyed':''}`}
            style={{backgroundImage:`url(${sprites})`}}
            onClick={()=>{ props.withinR && props.clickR() }}
            func={ key => { key === 'enter' && props.withinR && props.clickR() }}
        />
    </Nav>
)
...
function Settings (props) {
    ...
    {mod, modDscrt, modBool, modSpec, modVal, modColor, settingChanged, tabs, activeTab, switchTab, playProfileInd, onFocus, onBlur, showProfiles, setProfileInd,buttons,statsProps} = props,
    ...
    return (<div className='menu settings'>
      <div className='menubox'>
        ...
        <Nav className="menubox across TabBar"
            ...
            func={(key, navTree, focusedNode) => !navTree.focusedNode ? activeTab + '' : navHorizontal(key, navTree, focusedNode)}
        >
            <div className={`arrowL ${tabs[activeTab] === 'gameplay' ? 'greyed':''}`} style={{backgroundImage:`url(${sprites})`}}/>
            <Nav navId='0' className={`Tab ${tabs[activeTab] === 'gameplay' ? 'hovering':''}`}>Gameplay</Nav>
            <Nav navId='1' className={`Tab ${tabs[activeTab] === 'personal' ? 'hovering':''}`}>Profile</Nav>
            <div className={`arrowR ${tabs[activeTab] === 'personal' ? 'greyed':''}`} style={{backgroundImage:`url(${sprites})`}}/>
        </Nav>
        {tabs[activeTab] === 'personal' &&
            <div className="menubox across personal">
                {showProfiles && <div className="scrollContainer">
                    <div className="formGrid">
                        {Profile.cosm.map((p,i)=>(
                            <div key={i} className={`pfp ${i === playProfileInd ? 'active':''}`} loading='lazy' style={{backgroundImage: `url(${p.img})`}} onClick={()=>{setProfileInd(i);buttons[1].onClick()}}/>
                        ))}
                    </div>
                </div>}
                {!showProfiles && <div className="scrollContainer">
                    <div className='menubox stats'>
                        ...
                        <div className="kbutton" onClick={() => buttons[1].onClick()}>{buttons[1].text}</div>
        ...
        { tabs[activeTab] === 'gameplay' &&
            <Nav className="menubox settingsList" func={navVertical}>
                <div className="settingsItem">
                    <div className='subtitle'>Number of Rows</div>
                    <NavCounter withinL={tubLenInLimit.l} withinR={tubLenInLimit.r} clickL={()=>mod('tubLen',-1)} clickR={()=>mod('tubLen',1)} value={tubLen}/>
                </div>
                <div className="settingsItem">
        ...
        <KButton text={settingChanged ? 'Save' : 'Back'} onClick={buttons[0].onClick} navId="settingsReturn" hasSpacer/>
      </div>
    </div>)
}
